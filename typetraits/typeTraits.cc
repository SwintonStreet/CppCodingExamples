...
{
    return std::is_integral<t1>::value &&
           std::is_same<t1,t2>::value? type1 + type2 : type1;
}
...
int main()
{
    ...
    std::cout << "t1 trivially constructable: " << std::is_trivially_constructible<testType>::value << '\n';
    std::cout << "t1 empty                  : " << std::is_empty<testType>::value << '\n';
